...
def run_scrape_letter(exchange: str, letter: str, status: str = "listed") -> str:
    ...
    headers = {
        ...
        "Referer": urljoin(
            BASE_URL, "/en/listings/listing-with-us/listed-company-directory"
        ),
    }
    try:
        ...
        try:
            ...
            for item in data.get("results", []):
                ...
                if not instruments:
                    ...
                    listing_url = (
                        f"https://money.tmx.com/en/quote/{sym}/" if sym else None
                    )
                    if sym:
...
def run_scrape_status_page(exchange: str, status: str = "delisted") -> str:
    ...
    try:
        resp = session.get(
            urljoin(BASE_URL, LISTING_PATH),
            params={"exchange": exch_key, "action": status},
            headers=headers,
            timeout=30,
        )
    ...
    rows = (
        soup.select("table#tresults tbody tr")
        or soup.select("table tbody tr")
        or soup.select(".listed-company-directory__result-row")
    )
    if not rows:
    ...
    try:
        ...
        for r in rows:
            ...
            if not symbol:
                continue
            listing_url = (
                urljoin(BASE_URL, a["href"]) if a and a.has_attr("href") else None
            )
            status_date = None
            if len(tds) >= 3:
            ...
            if status == "delisted":
                ...
                """,
                    (
                        ...
                        listing_url,
                        status,
                        status not in ("delisted", "suspended"),
                        status_date,
                        "STOCK",
                    ),
                )
    ...
    try:
        ...
        if reader:
            ...
            for item in arr:
                sym = (item.get("symbol") or item.get("Symbol") or "").strip()
                name = (
                    item.get("name") or item.get("Name") or item.get("company") or ""
                ).strip()
                if not sym:
                    continue
                cur.execute(
