...
class Listing(models.Model):
    ...
    status = models.CharField(
        max_length=16, choices=STATUS_CHOICES, default="listed", db_index=True
    )
    ...
    status_date = models.DateField(
        null=True,
        blank=True,
        help_text="Date when status (delisted/suspended) occurred",
    )
...
class DelistedListing(models.Model):
    ...
    exchange = models.CharField(
        max_length=8, choices=Listing.EXCHANGE_CHOICES, db_index=True
    )
    symbol = models.CharField(max_length=32, db_index=True)
    name = models.CharField(max_length=255)
    listing_url = models.URLField(null=True, blank=True)
...
class Sector(models.Model):
    ...
    sector_code = models.CharField(
        max_length=10, blank=True, null=True, help_text="GICS sector code"
    )
...
class GeographicRegion(models.Model):
    ...
    country_code = models.CharField(
        max_length=8, blank=True, null=True, help_text="ISO country code"
    )
    region_type = models.CharField(
        max_length=20, choices=REGION_TYPE_CHOICES, blank=True, null=True
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class ETFInfo(models.Model):
    ...
    isin = models.CharField(
        max_length=32, blank=True, null=True, help_text="International Securities ID"
    )
    ...
    assets_under_management = models.BigIntegerField(
        blank=True, null=True, help_text="Total AUM in dollars"
    )
    inception_date = models.DateField(blank=True, null=True)
    fund_family = models.CharField(
        max_length=100, blank=True, null=True, help_text="e.g., Vanguard, iShares"
    )
    category = models.CharField(
        max_length=100, blank=True, null=True, help_text="e.g., Equity, Bond, Balanced"
    )
    ...
    domicile = models.CharField(
        max_length=8, default="CA", help_text="Country of domicile"
    )
...
class StockDetail(models.Model):
    ...
    listing = models.OneToOneField(
        Listing, on_delete=models.CASCADE, related_name="detail"
    )
    ...
    market_cap = models.BigIntegerField(
        blank=True, null=True, help_text="Market capitalization in dollars"
    )
    industry = models.CharField(
        max_length=150, blank=True, null=True, help_text="More specific than sector"
    )
...
class ETFHolding(models.Model):
    ...
    stock_listing = models.ForeignKey(
        Listing, on_delete=models.CASCADE, related_name="etf_holdings"
    )
    ...
    shares_held = models.BigIntegerField(
        blank=True, null=True, help_text="Number of shares held"
    )
    market_value = models.BigIntegerField(
        blank=True, null=True, help_text="Market value of holding in dollars"
    )
...
class ETFSectorAllocation(models.Model):
    ...
    etf = models.ForeignKey(
        ETFInfo, on_delete=models.CASCADE, related_name="sector_allocations"
    )
...
class ETFGeographicAllocation(models.Model):
    ...
    etf = models.ForeignKey(
        ETFInfo, on_delete=models.CASCADE, related_name="geographic_allocations"
    )
...
class YFinanceSectorCache(models.Model):
    ...
    sector_name = models.CharField(
        max_length=100, help_text="Human-readable sector name"
    )

    # Core sector information
    symbol = models.CharField(max_length=20, blank=True, null=True)
    overview = models.TextField(
        blank=True, null=True, help_text="Sector overview/description"
    )
    ...
    top_etfs_data = models.JSONField(
        blank=True, null=True, help_text="Cached top ETFs data"
    )
    top_companies_data = models.JSONField(
        blank=True, null=True, help_text="Cached top companies data"
    )
    top_mutual_funds_data = models.JSONField(
        blank=True, null=True, help_text="Cached mutual funds data"
    )
    industries_data = models.JSONField(
        blank=True, null=True, help_text="Cached industries data"
    )
    research_reports_data = models.JSONField(
        blank=True, null=True, help_text="Cached research reports"
    )

    # Cache metadata
    data_fetched_at = models.DateTimeField(
        auto_now_add=True, help_text="When data was fetched from yfinance"
    )
    last_updated = models.DateTimeField(
        auto_now=True, help_text="Last update timestamp"
    )
    fetch_error = models.TextField(
        blank=True, null=True, help_text="Last fetch error if any"
    )
    fetch_success = models.BooleanField(
        default=True, help_text="Whether last fetch was successful"
    )
...
class EnrichedTickerData(models.Model):
    ...
    symbol = models.CharField(
        max_length=32, db_index=True, help_text="Stock symbol (e.g., AAPL, SHOP.TO)"
    )
    exchange = models.CharField(
        max_length=20, blank=True, null=True, help_text="Stock exchange"
    )
    company_name = models.CharField(
        max_length=200, blank=True, null=True, help_text="Company name"
    )
    ...
    asset_type = models.CharField(
        max_length=20, choices=ASSET_TYPE_CHOICES, default="OTHER", db_index=True
    )
    asset_confidence = models.FloatField(
        default=0.0, help_text="Confidence score for asset classification (0-1)"
    )
    ...
    industry_key = models.CharField(
        max_length=50, blank=True, null=True, help_text="yfinance industry key"
    )
    ...
    # Market data
    market_cap = models.BigIntegerField(
        blank=True, null=True, help_text="Market capitalization in USD"
    )
    currency = models.CharField(
        max_length=10, blank=True, null=True, help_text="Trading currency"
    )
    is_active = models.BooleanField(
        default=True, help_text="Whether the ticker is actively trading"
    )

    # Data quality and source tracking
    data_source = models.CharField(
        max_length=50, default="yfinance", help_text="Primary data source used"
    )
    data_quality_score = models.FloatField(
        default=0.0, help_text="Overall data completeness score (0-1)"
    )
    fetch_success = models.BooleanField(
        default=True, help_text="Whether data fetch was successful"
    )
    fetch_errors = models.JSONField(
        blank=True, null=True, help_text="Any errors encountered during fetch"
    )

    # Timestamp tracking for change detection
    first_loaded_at = models.DateTimeField(
        auto_now_add=True, help_text="When this ticker was first processed"
    )
    last_updated_at = models.DateTimeField(
        auto_now=True, help_text="When this record was last updated"
    )
    last_checked_at = models.DateTimeField(
        auto_now=True, help_text="When this ticker was last checked by DAG"
    )
    data_changed_at = models.DateTimeField(
        blank=True, null=True, help_text="When the ticker data last actually changed"
    )
    ...
    version = models.IntegerField(
        default=1, help_text="Version number, incremented on changes"
    )
...
# ====================================================================
# Portfolio Models - TFSA/FHSA Account Tracking
# ====================================================================
...
class Portfolio(models.Model):
    ...
    institution = models.CharField(max_length=100, default="Wealthsimple")
    account_number = models.CharField(
        max_length=50, blank=True
    )  # Not unique - can have multiple imports for same account
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Transaction(models.Model):
    ...
    statement_period = models.CharField(max_length=20, blank=True)
    ...
    def __str__(self):
        return f"{self.symbol} {self.transaction_type} {self.date}"


class PortfolioHolding(models.Model):
    """Current holdings extracted from portfolio statement."""
    ...
    segregated_quantity = models.DecimalField(
        max_digits=18, decimal_places=8, null=True, blank=True
    )
    ...
    book_cost = models.DecimalField(max_digits=18, decimal_places=2)
    statement_period = models.CharField(max_length=20, blank=True)
    imported_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return f"{self.portfolio.name} - {self.symbol}: {self.quantity} shares"


class PortfolioCashSummary(models.Model):
    """Cash summary extracted from portfolio statement."""

    portfolio = models.ForeignKey(
        Portfolio, on_delete=models.CASCADE, related_name="cash_summaries"
    )
    last_statement_cash_balance = models.DecimalField(
        max_digits=18, decimal_places=2, default=Decimal("0")
    )
    total_cash_paid_in = models.DecimalField(
        max_digits=18, decimal_places=2, default=Decimal("0")
    )
    total_cash_paid_out = models.DecimalField(
        max_digits=18, decimal_places=2, default=Decimal("0")
    )
